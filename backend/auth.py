...
# Обязательный текущий пользователь
async def get_current_user(
    ...
    print(f"🔍 Получен токен: {token[:50]}..." if token else "🔍 Токен отсутствует")
    ...
    try:
        ...
        print(f"✅ Токен декодирован, username: {username}")
        if username is None:
            raise credentials_exception
    ...
    if user is None:
        print(f"❌ Пользователь {token_data.username} не найден")
        raise credentials_exception
